...
class OutputMixin:
    ...
    def print_status(swath_nr: int, areas: dict[str, Any]) -> None:
        ...
        print(
            ...
            f'sabkha: {areas["area_sabkha"]:.2f}\r',
            end="",
        )
    ...
    def print_totals(
        gis: Gis,
        total_src_area: float,
        total_src_sabkha_area: float,
        total_src_dune_area: float,
        total_rcv_area: float,
    ...
    def stats_to_excel(self, cfg: Config) -> None:
        ...
        while True:
            try:
                ...
                print(
                    f"Unable to write the excel file, please close {cfg.excel_file} ..."
                )
                time.sleep(5)

        writer = pd.ExcelWriter(
            cfg.excel_file, engine="xlsxwriter"
        ...
        self.swath_src_stats.loc["totals"] = pd.concat(
            [
                self.swath_src_stats.iloc[:, 1:18].sum(),
                self.swath_src_stats.iloc[:, 18:].mean(),
            ]
        )
        self.swath_src_stats.iloc[-1, 0] = "Totals"
        self.swath_src_stats.to_excel(writer, sheet_name="Source", index=False)

        self.swath_rcv_stats.loc["totals"] = pd.concat(
            [
                self.swath_rcv_stats.iloc[:, 1:9].sum(),
                self.swath_rcv_stats.iloc[:, 9:10].mean(),
                self.swath_rcv_stats.iloc[:, 10:].sum(),
            ]
        )
        self.swath_rcv_stats.iloc[-1, 0] = "Totals"
        self.swath_rcv_stats.to_excel(writer, sheet_name="Receiver", index=False)
        self.prod_stats.loc["totals"] = pd.concat(
            [self.prod_stats.iloc[:, 4:20].sum(), self.prod_stats.iloc[:, 20:].mean()]
        )
        self.prod_stats.iloc[-1, 0] = "Totals"
        ...
        chart1.set_title(
            {
                "name": cfg.title_chart + " - VP by type",
        ...
        chart2.set_title(
            {
                "name": cfg.title_chart + " - CTM by swath",
                "name_font": name_font_title,
            }
        )
        ...
        chart3.set_title(
            {
                "name": cfg.title_chart + " - Source density by swath",
                "name_font": name_font_title,
            }
        )
        ...
        chart5.set_title(
            {
                "name": cfg.title_chart + " - Cumul. dozer km",
                "name_font": name_font_title,
            }
        )
        ...
        chart7.set_title(
            {
                "name": cfg.title_chart + " - Production",
                "name_font": name_font_title,
            }
        )
